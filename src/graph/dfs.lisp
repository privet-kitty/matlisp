...
(defmacro-clause (FOR v IN-GRAPH g &optional FROM root WITH-ORDER order WITH-COLOR color WITH-PARENT p)
  (binding-gensyms (gm gf)
      ...
      (let* ((pushor `(letv* ((,(gm l) ,(gm r) (fence ,(gm g) ,v) :type index-type index-type))
			(iter (for ,(gm u) in-vector (δ-i ,(gm g)) from ,(gm l) below ,(gm r)) (with ,(gm switch) = nil) (finally (return ,(gm switch)))
			      (unless (aref ,visited ,(gm u)) (setf ,(gm switch) t)
				      ,@(ecase order
					       (:dfs `((push ,(gm u) ,(gm stack))
						       ,@(if p `((push ,v ,(gm pstack))))))
					       (:bfs `((setf ,(gm stack) (matlisp-dlist:dcdr (matlisp-dlist:dpush ,(gm u) ,(gm stack)))
							     ,@(if p `(,(gm pstack) (matlisp-dlist:dcdr (matlisp-dlist:dpush ,v ,(gm pstack))))))))
					       (:sfd `((push ,(gm u) ,(gm stack)))))))))
	     (poppor `(iter (repeat (dimensions ,(gm g) 1)) (unless ,(gm stack) (finish))
			    (let ((,(gm v) ,(ecase order ((:dfs :sfd) `(pop ,(gm stack))) (:bfs `(matlisp-dlist:dpop ,(gm stack)))))
				  ,@(if (and p (not (eql order :sfd))) `((,(gm p) ,(ecase order (:dfs `(pop ,(gm pstack))) (:bfs `(matlisp-dlist:dpop ,(gm pstack))))))))
	...
	`(progn
	   ...
	   (with ,(gm stack) = nil)
	   ,@(if (and p (not (eql order :sfd))) `((with ,(gm pstack) = nil) (with ,p = nil)))
	   ,@(if (eql order :sfd)
