...
(with-memoization ((trivial-garbage:make-weak-hash-table :weakness :key-or-value :test 'equal))
  (defmem minors-precompute! (dims std indices)
...
;;I have no idea what the preconditions for this should be. May want to add
;;support for contiguous indices.
(define-tensor-method minors! ((x dense-tensor :x) (y dense-tensor :y) &rest indices)
  `(letv* ((inc-x stable (minors-precompute! (lvec->list (dimensions x)) (lvec->list (strides x)) indices) :type index-type simple-vector)
     ...
     (macrolet ((sv (i) `(the index-store-vector (aref stable ,i))))
       (iter (with idx := (t/store-allocator index-store-vector (order y)))
	     ...
	     (with ref-x := (+ inc-x (head x))) (with ref-y := (head y))
