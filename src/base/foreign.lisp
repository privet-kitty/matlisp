	...
	(using-gensyms (decl (store idx) (2idx))
	  `(let (,@decl)
	     ...
	     (let-typed ((,2idx (* 2 ,idx) :type index-type))
	       (values (complex (ffi:fvref (the ,(store-type class) ,store) ,2idx) (ffi:fvref (the ,(store-type class) ,store) (1+ ,2idx))) t))))
	`(values (ffi:fvref (the ,(store-type class) ,store) (the index-type ,idx)) t))))
	...
	(using-gensyms (decl (store idx value) (2idx))
	  `(let (,@decl)
	     (declare (type ,(store-type class) ,store)
		      ...
		      (type index-type ,idx))
	     (let-typed ((,2idx (* 2 ,idx) :type index-type))
	       (funcall #'(setf ffi:fvref) (the ,real-type (cl:realpart ,value)) (the ,(store-type class) ,store) ,2idx)
	       (funcall #'(setf ffi:fvref) (the ,real-type (cl:imagpart ,value)) (the ,(store-type class) ,store) (1+ ,2idx)))
	     ,value))
	`(funcall #'(setf ffi:fvref) (the ,(field-type class) ,value) (the ,(store-type class) ,store) (the index-type ,idx)))))
