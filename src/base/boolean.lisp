...
(closer-mop:defmethod ga= ((a null) (b #.(tensor 'boolean)))
  ...
  (let ((ret t))
    (with-memoization ()
      (iter (for-mod idx from 0 below (dimensions b) with-iterator ((:stride ((of-b (strides b) (head b))))))
	    (unless (t/store-ref #.(tensor 'boolean) (memoizing (store b) :type #.(store-type (tensor 'boolean))) of-b)
	      (setf ret nil)
	      (return))))
    ret))
;;
...
(closer-mop:defmethod go= ((a null) (b #.(tensor 'boolean)))
  ...
  (let ((ret nil))
    (with-memoization ()
      (iter (for-mod idx from 0 below (dimensions b) with-iterator ((:stride ((of-b (strides b) (head b))))))
	    (when (t/store-ref #.(tensor 'boolean) (memoizing (store b) :type #.(store-type (tensor 'boolean))) of-b)
	      (setf ret t)
	      (return))))
    ret))
