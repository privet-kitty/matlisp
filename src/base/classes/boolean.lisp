...
(eval-every
  ...
  (deft/method (t/store-ref #'(lambda (x) (eql (field-type x) 'boolean))) (type simple-vector-store-mixin) (store &rest idx)
    (with-gensyms (out)
      `(let-typed ((,out (t/store-ref ,(dense-tensor 'bit) ,store ,@idx) :type bit))
  ...
  (deft/method (t/store-set #'(lambda (x) (eql (field-type x) 'boolean))) (type simple-vector-store-mixin) (value store &rest idx)
    `(setf (t/store-ref ,(dense-tensor 'bit) ,store ,@idx) (the bit (if ,value 1 0)))))
;;
...
(define-tensor-method ge= (a (b dense-tensor :x))
  `(let ((a (t/coerce ,(field-type (cl :x)) a))
	 (ret (zeros (dimensions b) (dense-tensor 'boolean))))
     (dorefs (idx (dimensions b))
	     ((ref.b b :type ,(cl :x))
	      (ref.r ret :type ,(dense-tensor 'boolean)))
	     (setf ref.r (t/f= ,(field-type (cl :x)) a ref.b)))
     ret))
(define-tensor-method ge= ((a dense-tensor :x) (b dense-tensor :x))
  `(let ((ret (zeros (dimensions a) (dense-tensor 'boolean))))
  ...
  `(let ((a (t/coerce ,(field-type (cl :x)) a))
     ...
     (with-memoization ()
       (iter (for-mod idx from 0 below (dimensions b) with-iterator ((:stride ((of-b (strides b) (head b))))))
	     (unless (t/f= ,(field-type (cl :x)) a (t/store-ref ,(cl :x) (memoizing (store b) :type ,(store-type (cl :x))) of-b))
	       (setf ret nil)
	       (return))))
     ret))
     ...
     (with-memoization ()
       (iter (for-mod idx from 0 below (dimensions b) with-iterator ((:stride ((of-a (strides a) (head a))
	     ...
	     (unless (t/f= ,(field-type (cl :x))
	       ...
	       (return))))
     ret))
(defmethod ga= ((a null) (b #.(dense-tensor 'boolean)))
  (declare (ignore a))
  (let ((ret t))
    (with-memoization ()
      (iter (for-mod idx from 0 below (dimensions b) with-iterator ((:stride ((of-b (strides b) (head b))))))
	    (unless (t/store-ref #.(dense-tensor 'boolean) (memoizing (store b) :type #.(store-type (dense-tensor 'boolean))) of-b)
	      (setf ret nil)
	      (return))))
    ret))
;;
...
(define-tensor-method go= (a (b dense-tensor :x))
  `(let ((a (t/coerce ,(field-type (cl :x)) a))
     ...
     (with-memoization ()
       (iter (for-mod idx from 0 below (dimensions b) with-iterator ((:stride ((of-b (strides b) (head b))))))
	     (when (t/f= ,(field-type (cl :x)) a (t/store-ref ,(cl :x) (memoizing (store b) :type ,(store-type (cl :x))) of-b))
	       (setf ret t)
	       (return))))
     ret))
     ...
     (with-memoization ()
       (iter (for-mod idx from 0 below (dimensions b) with-iterator ((:stride ((of-a (strides a) (head a))
	     ...
	     (when (t/f= ,(field-type (cl :x))
	       ...
	       (return))))
     ret))
(defmethod go= ((a null) (b #.(dense-tensor 'boolean)))
  (declare (ignore a))
  (let ((ret nil))
    (with-memoization ()
      (iter (for-mod idx from 0 below (dimensions b) with-iterator ((:stride ((of-b (strides b) (head b))))))
	    (when (t/store-ref #.(dense-tensor 'boolean) (memoizing (store b) :type #.(store-type (dense-tensor 'boolean))) of-b)
	      (setf ret t)
	      (return))))
    ret))
;; (zeros 2 (dense-tensor 'boolean))
;; (go= (ones 2) #d[1, 0])
