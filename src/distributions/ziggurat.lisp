...
(defun ziggurat-compile (points function
  ...
  (letv* ((log-nz (floor (log n-ziggurat 2)))
    ...
    (using-gensyms (decl (r0 ktab ytab wtab))
      `(let (,@decl)
	 (declare (type (simple-array (unsigned-byte ,random-byte-size) (,n-ziggurat)) ,ktab)
		  ...
		  (type ,(type-of identity) ,r0))
	 (lambda ()
	   (loop
	      (let* ((rint (,byte-sampler ,(expt 2 random-byte-size)))
		...
		(if (< ii (1- n-ziggurat))
		    (let ((y0 (aref ,ytab ii)) (y1 (aref ,ytab (1+ ii)))
		      ...
		      (setf yy (+ y1 (* (- y0 y1) U1))))
		    (setf (values xx yy) (,slow-sampler ,r0 ,(funcall function r0))))
		(when (< yy (,function xx)) (return (* xx ,@(if symmetricp `(ss))))))))))))
