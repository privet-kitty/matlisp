...
(defun graph->dot (stream g &optional self-loops node-name)
  (if (null stream)
	...
	(iter (for u from 0 below (dimensions g -1))
		...
		(iter (for v in-vector (neighbors g) from lb below ub)
		      (when (or (and (= u v) (not self-loops)) (and (< v u) (neighbors g v u))) (next-iteration))
		      (format stream (string+ "~a->~a [" (if (neighbors g v u) "dir=none,color=red," "")  "label=\"\",];~%") v u))))
	(format stream "}~%"))))
