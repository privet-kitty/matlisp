...
(let ((code `(lambda (val lb ub vec &key (order #'<) (test #'=))
  ...
  (define-compiler-macro binary-search (&whole form val lb ub vec &key (order '(function <)) (test '(function =)))
    (letv* ((vtype-givenp (and (consp vec) (eql (first vec) 'the)))
	    ((inline-orderp inline-testp) (mapcar #'(lambda (x) (and (consp x) (member (first x) '(quote lambda function)))) (list order test))))
	  ...
	  (let ((code (maptree-if #'(lambda (x) (and (consp x) (or (and vtype-givenp (equal x '(type vector vec)))
				  ...
				  #'(lambda (x)
					  ...
					  `(type ,(second vec) vec)
					  (let ((code (ecase (second x) (order (and inline-orderp order)) (test (and inline-testp test)))))
					    (if code
						(if (member (first code) '(quote function)) `(,(second code) ,@(cddr x))
						    `(let (,@(zip (second code) (cddr x)))
						       ,@(cddr code)))
						x))))
				  (cddr code)))
